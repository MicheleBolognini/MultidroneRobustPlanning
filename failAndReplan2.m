%% Order Edges as they are traversed
orderedEdges = {};
for i = 1:numv
...
% failDrone = randi(numv) % This drone fails
% % It fails while inspecting the second node of the followng edge, i.e. does
% % not visit it completely
% failEdge = randi(length(orderedEdges{failDrone})-1)

% Always fail with the drone whose span is longest
failDrone = 1;
for i = 1:length(orderedEdges)
    if length(orderedEdges{i}) > length(orderedEdges{failDrone})
        failDrone = i;
    end
end
%failEdge = randi(length(orderedEdges{failDrone})-1);
%failEdge = randi(2); % Always fail within the first two edges
failEdge = 2; %Always fail after the first visited node
...
newCapacities = params.droneCapacity*ones(numv,1) - traversedLengths; % Updated capacities = previous - travelled
...
% Each vehicle cannot span more tha params.droneCapacity meters
for k = 1:numv
    row = [];
    for j = 1:k-1
        row = [row, zeros(1, length(edges_reduced))];
